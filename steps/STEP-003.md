...
- Step 002에서 만든 mock 지구본 날씨 오버레이를 Open-Meteo 현재 날씨 데이터와 연결한다.
- 대표 좌표 기반 요청만 사용해 모든 국가를 계속 호출하는 구조를 피한다.
- API 응답을 UI에서 직접 쓰지 않고 앱 내부 `WeatherOverlayPoint[]` 데이터로 정규화한다.
...
- 대시보드에서 오버레이 데이터를 불러오고, 로딩/에러/빈 상태를 지도 주변에 표시한다.
- API 실패 시 지구본 선택 경험이 막히지 않도록 기존 mock 데이터를 fallback으로 유지한다.
...
- 지구본 오버레이는 실제 API 데이터가 있으면 해당 데이터를 사용하고, 실패 시 mock 데이터를 유지한다.

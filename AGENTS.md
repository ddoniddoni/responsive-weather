...
이 프로젝트는 작업을 Step 단위로 나누어 진행한다.

- 기능 개발은 한 번에 크게 진행하지 않고, Step을 하나씩 완료한다.
- 각 Step을 시작하기 전에 목표/범위/완료 기준을 먼저 작성한다.
- Step 계획 문서는 루트 `steps/` 폴더에 기록한다.
- 파일명은 `STEP-001.md`, `STEP-002.md`처럼 순차 번호를 사용한다.
- Step 문서에는 최소한 다음 내용을 포함한다.
  - 목표
  - 작업 범위
  - 완료 기준
  - 작업 체크리스트
...
## 32. Step 완료 후 Git Push 규칙

각 Step이 완료될 때마다 커밋 후 원격 저장소에 push 한다.

- 원칙적으로 Step당 1개 커밋을 유지한다.
- Step 시작 시 전용 브랜치를 생성한다. (예: `step-003-weather-button-effects`)
- Step 작업은 전용 브랜치에서 진행하고, 완료 후 원격에도 해당 브랜치를 push 한다.
- 여러 변경이 섞이면 Step 경계에 맞춰 커밋을 분리한다.
- push 전 가능한 경우 lint/build 등 검증 결과를 확인한다.
...
- `refactor`: 코드 리팩토링 (기능 변경/버그 수정 제외)
- `test`: 테스트 코드 추가/수정
